...
Open via **Manage Connections** in the toolbar.
...
|-------|-------------|---------|
| Name | Display name (optional, auto-generated from host/database if empty) | `Production DB` |
| **Connection Type** | Hosting platform — auto-detected from hostname, or select manually | `Supabase` / `Aiven` / `Neon` / `PostgreSQL` |
| Host | Server hostname or IP | `db.example.com` |
| Port | PostgreSQL port (auto-filled when you select a Connection Type) | `5432` |
| Database | Database name | `my_app` |
| Username | Login user | `postgres` |
| Password | Login password | `••••••••` |
| SSL Mode | Connection security (auto-filled when you select a Connection Type) | `Prefer` / `Require` |

### Connection Type Auto-Detection

When you paste a hostname, DbClone automatically detects the platform from the hostname pattern and sets the **Connection Type** dropdown accordingly:
...
Changing the Connection Type updates the port and SSL mode to the platform's defaults. You can override these values after the auto-fill.

!!! tip "Paste a URI"
    You can paste a full connection URI in the host field:
    ```
    postgres://user:password@host:5432/dbname?sslmode=require
    ```
    DbClone will parse it, fill all fields, and auto-detect the platform from the hostname.

## Testing a Connection

Click **Test** to verify the connection works. DbClone will:
...
Organize connections into folders (e.g., "Production", "Staging", "Local"). Assign a folder when creating or editing a connection to group it in the list.

## Editing & Deleting

- Select a connection and modify the fields, then click **Save** to update
- Click **Delete** to remove a connection (requires confirmation)
- Click **Duplicate** to create a copy of the current connection with a new name
